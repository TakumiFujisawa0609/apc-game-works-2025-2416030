...
float AsoUtility::GetDeltaTime()
{
    ...
    if (prevTime == 0) prevTime = nowTime; // 初回だけ補正

    float deltaTime = (float)(nowTime - prevTime) / 1000000.0f; // 秒
    prevTime = nowTime;

    // 安全対策（FPSが極端に落ちた時のワープ防止）
    if (deltaTime > 0.033f) deltaTime = 0.033f; // 1/30秒が上限
